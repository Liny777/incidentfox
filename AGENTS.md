...
```
Slack  → slack-bot (Bolt/Socket Mode) ─→ sre-agent (Codex Agent SDK) → gVisor sandbox
Web UI ─────────────────────────────────↗        ↕
...
**sre-agent** is the active agent. Runs in isolated gVisor K8s sandbox pods — each investigation gets its own sandbox. Uses Codex SDK with 45 skills (progressive knowledge loading) and scripts (Python/Bash integrations). No MCP tools — everything is skills + scripts.
...
The history: agent/ (OpenAI SDK) → sre-agent (Codex SDK) → unified-agent (OpenHands SDK, deleted) → back to sre-agent (Codex SDK). We standardized on sre-agent because its skills architecture is simpler and Codex SDK is better tested.
...
| File | What it does |
|------|-------------|
| sre-agent/agent.py | Codex SDK agent with skills, streaming, subagents |
...
**Skills over tools**: sre-agent uses `.Codex/skills/*/SKILL.md` for progressive knowledge loading (~100 tokens metadata, full content on demand). Add new integrations as skills with scripts, not as Python tool functions.
...
## Documentation Governance (Liny777 fork)

This fork has a documentation governance system under `document/`. **All process documents — plans, reviews, implementation notes, verifications, post-mortems, ops guides, knowledge notes — go into `document/<XX-category>/`, never into the repo root or into code directories.**

Before writing any process document:

1. Read [`document/01-rules/doc-governance.md`](document/01-rules/doc-governance.md) — the source of truth for frontmatter, `doc_type` enum, and `DOC-IFX-<TYPE>-<YYYYMMDD>-<NNN>` naming.
2. Pick the right subdirectory by `doc_type` (see the §2 table in `doc-governance.md`).
3. Copy the matching template from [`document/01-rules/templates/`](document/01-rules/templates/) (8 templates: plan / review / implementation / verify / postmortem / ops-guide / knowledge / meta-sidecar).

After writing:

4. Update the affected directory's `README.md` (every directory under `document/` has a `doc_type: 目录索引` README that lists its docs).
5. Run `bash document/01-rules/scripts/validate_docs_metadata.sh`. Exit 0 = PASS. Anything else must be fixed before committing.

Non-Markdown assets (`.json`, `.yaml`, `.drawio`, `.png`, `.sh`, etc.) placed inside `document/` require a sibling `<name>.<ext>.meta.md` sidecar — see [`document/01-rules/templates/meta-sidecar.md`](document/01-rules/templates/meta-sidecar.md).

Upstream files in the repo root (`README.md`, `CLAUDE.md`, this `AGENTS.md`, `LICENSE*`, `CONTRIBUTING.md`, `SECURITY.md`, etc.) are *not* governed by this system; leave them alone unless the task is specifically to update them.

Git workflow rules (branch naming, PR flow, upstream sync) live at [`document/01-rules/git-workflow.md`](document/01-rules/git-workflow.md).
